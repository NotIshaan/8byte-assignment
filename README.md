...
The application itself is intentionally minimal - a small Express app with a '/health'endpoint, a '/ready' endpoint that checks DB connectivity, and basic todo CRUD routes. The infrastructure is the main project.
...
- **EC2** - runs inside an Auto Scaling Group with a minimum of 1 instance. On boot, pulls the latest Docker image from ECR via a userdata  script and run it as a container.
...
- **NAT Gateway** *(happy path)* - allows EC2 instances in private subnets to reach ECR, CloudWatch and Secrets Manager without being exposed to the internet. VPC Endpoints per service would be cheaper but adds more resources to manage - documented under Cost optimization section.
...
RDS automated backups are enabled with a 7 day retention. AWS takes a daily snapshot automatically and keeps the last 7 days.
...
**Flat .tf files over Terraform modules** *(happy path)* - kept all Terraform in flat `.tf` files (vpc.tf, ec2.tf etc) rather than separate module folders. Modules would give better single responsibility and reusability but add significant boilerplate for an assignment. In production each infra part would be its own module with its own variables and outputs.

**Local Terraform state** *(happy path)* - state is stored locally instead of remotely in S3 with DynamoDB locking. For production this would be the first thing to set up before anything else because your terraform state would be gone if your laptop dies with local state.
...
**EC2 over ECS/EKS** - straightforward for this scale and easier to reason about end to end. ECS would be the natural next step for proper container orchestration without managing the underlying instances. maybe eks but that depends on what kind of app we're running

**No HTTPS on ALB** *(happy path)* - proper setup requires a domain name and certificates. Running on HTTP only for this assignment. Would never do this in production. 

**Trivy exit code 0** *(happy path)* - image is scanned for vulnerabilities but the build does not fail on findings. In production I would fail the build on CRITICAL severity findings.
